...
def ${b.FUNCTION_NAME_PLACEHOLDER_}(start, stop, step):
  while start <= stop:
    yield start
    start += abs(step)
`)}var e=b.getVariableName(a.getFieldValue("VAR")),f=b.valueToCode(a,"FROM",U.NONE)||"0",g=b.valueToCode(a,"TO",U.NONE)||"0",h=b.valueToCode(a,"BY",U.NONE)||"1",m=b.statementToCode(a,"DO");m=b.addLoopTrap(m,a)||b.PASS;var z="";$.df(f)&&$.df(g)&&$.df(h)?(f=Number(f),g=Number(g),h=Math.abs(Number(h)),f%1===0&&g%1===0&&h%1===0?(f<=g?(g++,a=f===0&&h===1?g:f+", "+g,h!==1&&(a+=", "+h)):(g--,a=f+", "+g+", -"+h),a="range("+a+")"):(a=f<g?d():c(),a+="("+f+", "+g+", "+h+")")):(a=function(E,G){$.df(E)?E=String(Number(E)):
E.match(/^\w+$/)||(G=b.nameDB_.getDistinctName(e+G,$.P.VARIABLE),z+=G+" = "+E+"\n",E=G);return E},f=a(f,"_start"),g=a(g,"_end"),h=a(h,"_inc"),typeof f==="number"&&typeof g==="number"?(a=f<g?d():c(),a+="("+f+", "+g+", "+h+")"):a="("+f+" <= "+g+") and "+d()+"("+f+", "+g+", "+h+") or "+c()+"("+f+", "+g+", "+h+")");return z+="for "+e+" in "+a+":\n"+m},controls_forEach:function(a,b){var c=b.getVariableName(a.getFieldValue("VAR")),d=b.valueToCode(a,"LIST",U.RELATIONAL)||"[]",e=b.statementToCode(a,"DO");
